...
import { projectMemoryDomainSpec } from "./spec.js";
export { projectMemoryDomainSpec, projectMemoryEntrySchema, projectMemoryIdSchema, projectMemoryRecordSchema, } from "./spec.js";
 ...
 */
export function ProjectMemoryId(id) {
    return id;
}
...
function resolveConfig(config) {
    ...
    if (!Number.isSafeInteger(maxEntriesPerWorkspace) || maxEntriesPerWorkspace < 1) {
        throw new TypeError('project-memory: maxEntriesPerWorkspace must be a positive safe integer');
    }
    if (!Number.isSafeInteger(maxEntryChars) || maxEntryChars < 1) {
        throw new TypeError('project-memory: maxEntryChars must be a positive safe integer');
    }
    return {
...
function resolveContent(content, maxChars) {
    ...
    if (normalized.length === 0)
        return rejected({ code: 'content-blank' });
    if (normalized.includes('\0'))
        return rejected({ code: 'content-nul' });
    const actualChars = Array.from(normalized).length;
    if (actualChars > maxChars) {
        return rejected({ code: 'content-too-long', maxChars, actualChars });
    }
        ...
        entries: {
            ...
            items: {
                ...
                properties: {
                    ...
                    origin: { type: 'string', enum: ['tool', 'command', 'ui'], required: true },
                },
            },
        },
    },
...
function renderToolValue(_args, value) {
    ...
    if (value.entries.length === 0)
        lines.push('No memories saved.');
    else
        value.entries.forEach((entry) => { lines.push(`- [${entry.id}] ${entry.content}`); });
...
function renderCommandSnapshot(value) {
    ...
    if (value.entries.length === 0)
        ...
        value.entries.forEach((entry, index) => { lines.push(`${index + 1}. ${entry.id} — ${entry.content}`); });
...
let ProjectMemoryService = (() => {
    ...
    return class ProjectMemoryService extends _classSuper {
        ...
        static inject = ['agents', 'storageDomain', 'workspaceRegistry', 'tools', 'commands', 'systemPrompt'];
        ...
        validateStoredRows(table) {
            for (const [path, record] of table.entries()) {
                if (record.entries.length > this.resolved.maxEntriesPerWorkspace) {
                    throw new Error(`project-memory: workspace '${path}' stores ${record.entries.length} entries; `
                        + `configured maximum is ${this.resolved.maxEntriesPerWorkspace}`);
                }
                for (const entry of record.entries) {
                    const chars = Array.from(entry.content).length;
                    if (chars > this.resolved.maxEntryChars) {
                        throw new Error(`project-memory: workspace '${path}' entry '${entry.id}' stores ${chars} characters; `
                            + `configured maximum is ${this.resolved.maxEntryChars}`);
                    }
                }
            }
        }
        ...
        list(request) {
            ...
            return Promise.resolve(workspace === undefined
                ? rejected({ code: 'workspace-unavailable', sessionId: request.sessionId })
        ...
        add(request) {
            return this.mutateForSession(request.sessionId, request.ifRevision, 'ui', {
                kind: 'add', content: request.content,
            });
        }
        /**
        ...
        update(request) {
            return this.mutateForSession(request.sessionId, request.ifRevision, 'ui', {
                kind: 'update', id: request.id, content: request.content,
            });
        }
        /**
        ...
        listForAgent(agent) {
            ...
            return workspace === undefined
                ? rejected({ code: 'workspace-unavailable', sessionId: agent.session.id })
                : success(this.readWorkspace(workspace));
        }
        ...
        mutateForSession(sessionId, ifRevision, origin, mutation) {
            ...
            if (workspace === undefined) {
                return Promise.resolve(rejected({ code: 'workspace-unavailable', sessionId }));
            }
        ...
        mutateForAgent(agent, ifRevision, origin, mutation) {
            const workspace = this.workspaceForAgent(agent);
            if (workspace === undefined) {
                return Promise.resolve(rejected({ code: 'workspace-unavailable', sessionId: agent.session.id }));
            }
            return this.mutateWorkspace(workspace, ifRevision, origin, mutation);
        }
        ...
        mutateWorkspace(workspace, ifRevision, origin, mutation) {
            if (!Number.isSafeInteger(ifRevision) || ifRevision < 0) {
                return Promise.resolve(rejected({
                    code: 'revision-conflict',
                    current: this.readWorkspace(workspace),
                }));
            }
            const content = mutation.kind === 'delete'
                ? undefined
                : resolveContent(mutation.content, this.resolved.maxEntryChars);
            if (content !== undefined && !content.ok)
                return Promise.resolve(content);
            return this.enqueue(async () => {
                const table = this.requireTable();
                const current = table.get(workspace.path);
                const currentSnapshot = snapshot(workspace, current, this.resolved);
                if (currentSnapshot.revision !== ifRevision) {
                    return rejected({
                        code: 'revision-conflict',
                ...
                switch (mutation.kind) {
                    case 'add': {
                        if (entries.length >= this.resolved.maxEntriesPerWorkspace) {
                            return rejected({
                    ...
                    case 'update': {
                        ...
                        if (index === -1) {
                            return rejected({ code: 'memory-not-found', id: mutation.id });
                        }
                        ...
                        nextEntries = entries.map((entry, entryIndex) => entryIndex === index
                            ? snapshotEntry({ ...entry, content: nextContent, updatedAt, origin })
                            : entry);
                        break;
                    }
                    case 'delete': {
                        if (!entries.some(entry => entry.id === mutation.id)) {
                            return rejected({ code: 'memory-not-found', id: mutation.id });
                        }
        ...
        emitChanged(workspaceId, revision) {
            try {
                this.ctx.emit('project-memory/changed', { workspaceId, revision });
            }
            catch (error) {
        ...
        enqueue(operation) {
            if (!this.mutationAdmissionOpen) {
                return Promise.reject(new Error('project-memory service is disposing'));
            }
        ...
        registerContext() {
            if (!this.resolved.injectContext)
                return;
            this.ctx.effect(() => this.ctx.systemPrompt.context({
                ...
                text: ({ agent }) => {
                    ...
                    return workspace === undefined
                        ? ''
                        : renderContext(workspace, this.requireTable().get(workspace.path));
                },
        ...
        registerTool() {
            this.ctx.effect(() => this.ctx.tools.register(defineTool({
                ...
                output: { schema: TOOL_OUTPUT_SCHEMA, render: renderToolValue },
                execute: async (args, exec) => {
                    const agent = exec.agent;
                    if (agent === undefined) {
                        throw new HarnessError('project_memory requires an agent-bound caller', 'PROJECT_MEMORY_AGENT_REQUIRED');
                    }
                    let result;
                    switch (args.action) {
                        case 'list':
                            if (args.revision !== undefined || args.id !== undefined || args.content !== undefined) {
                                throw new HarnessError('project_memory list accepts only action', 'PROJECT_MEMORY_INVALID_REQUEST');
                            }
                            result = this.listForAgent(agent);
                            break;
                        case 'add':
                            if (args.revision === undefined || args.content === undefined || args.id !== undefined) {
                                throw new HarnessError('project_memory add requires revision and content, without id', 'PROJECT_MEMORY_INVALID_REQUEST');
                            }
                            this.requireDirectHumanMutation(agent);
                            result = await this.mutateForAgent(agent, args.revision, 'tool', {
                                kind: 'add', content: args.content,
                            });
                            break;
                        case 'update':
                            if (args.revision === undefined || args.content === undefined || args.id === undefined) {
                                throw new HarnessError('project_memory update requires revision, id, and content', 'PROJECT_MEMORY_INVALID_REQUEST');
                            }
                            this.requireDirectHumanMutation(agent);
                            result = await this.mutateForAgent(agent, args.revision, 'tool', {
                                kind: 'update', id: ProjectMemoryId(args.id), content: args.content,
                            });
                            break;
                        case 'delete':
                            if (args.revision === undefined || args.id === undefined || args.content !== undefined) {
                                throw new HarnessError('project_memory delete requires revision and id, without content', 'PROJECT_MEMORY_INVALID_REQUEST');
                            }
                            this.requireDirectHumanMutation(agent);
                            result = await this.mutateForAgent(agent, args.revision, 'tool', {
                                kind: 'delete', id: ProjectMemoryId(args.id),
                            });
                            break;
                        default: return assertNever(args.action, 'project_memory action');
                    }
                    if (!result.ok) {
                        throw new HarnessError(describeFailure(result.error), `PROJECT_MEMORY_${result.error.code.toUpperCase().replaceAll('-', '_')}`);
                    }
        ...
        requireDirectHumanMutation(agent) {
            if (this.ctx.agents.get(agent.id) !== agent || agent.status !== 'running'
                ...
                throw new HarnessError('Project Memory mutations require the exact live top-level calling agent.', 'PROJECT_MEMORY_EXPLICIT_USER_REQUIRED');
            }
        ...
        async executeCommand(invocation) {
            ...
            if (!listed.ok)
                return { kind: 'error', text: describeFailure(listed.error) };
            if (input.length === 0)
                return renderCommandSnapshot(listed.value);
            let result;
            if (input.startsWith('add ')) {
                result = await this.mutateForAgent(invocation.agent, listed.value.revision, 'command', {
                    kind: 'add', content: input.slice(4),
                });
            }
            else if (input.startsWith('edit ')) {
                const match = /^edit\s+(\S+)\s+([\s\S]+)$/u.exec(input);
                if (match === null)
                    return { kind: 'error', text: 'Usage: /memory edit <id> <content>' };
                result = await this.mutateForAgent(invocation.agent, listed.value.revision, 'command', {
                    kind: 'update', id: ProjectMemoryId(match[1]), content: match[2],
                });
            }
            else if (input.startsWith('delete ')) {
                const id = input.slice(7).trim();
                if (id.length === 0 || id.includes(' '))
                    return { kind: 'error', text: 'Usage: /memory delete <id>' };
                result = await this.mutateForAgent(invocation.agent, listed.value.revision, 'command', {
                    kind: 'delete', id: ProjectMemoryId(id),
                });
            }
            else {
                return {
            ...
            return result.ok ? renderCommandSnapshot(result.value) : { kind: 'error', text: describeFailure(result.error) };
        }
        /** Require the domain table after service initialization. */
        requireTable() {
            if (this.table === undefined)
                throw new Error('project-memory domain is not started yet');
            return this.table;
        }
    };
})();
export { ProjectMemoryService };
